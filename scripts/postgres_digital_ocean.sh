ssh root@164.90.164.177

ocCVWro7fALsd824YnZ!TH-A


http://164.90.164.177/pgadmin4
...
# Dynamically modify the postgresql.conf to allow listening on all IPs
...
# Get the PostgreSQL version dynamically
PG_VERSION=$(psql --version | awk '{print $3}' | cut -d. -f1)

# Add or update the necessary lines in pg_hba.conf for local and remote connections
...
echo "local   all             all                                     md5" | sudo tee -a /etc/postgresql/16/main/pg_hba.conf

# Allow remote connections with password authentication (from any IP address)
