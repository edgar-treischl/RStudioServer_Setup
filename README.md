...
This script automates the installation of various software packages on
an Ubuntu-based server, including R, RStudio, Shiny Server, and Nginx,
as well as setting up security measures such as Fail2ban and a UFW
firewall.

### 01 Update the System

This part updates the package lists and upgrades installed packages to
their latest versions. If any of these steps fail, the script will
...
Installs various dependencies required for the installation of RStudio,
Shiny, Nginx, and other necessary libraries like SSL, Java, and
...
Adds the CRAN repository for R to Ubuntu’s package manager. Installs the
base R package and development tools.
...
Downloads and installs the latest version of RStudio Server. Removes
the `.deb` file after installation.
...
Installs the Shiny package from CRAN. Downloads and installs Shiny
Server. Removes the `.deb` file after installation.
...
Creates the Shiny app directory `/srv/shiny-server` and sets the correct
ownership and permissions for the Shiny user.

Creates directories for logging and sets up necessary log files for both
...
Configures logging for both RStudio Server and Shiny Server, defining
...
Configures log rotation for Shiny Server logs to ensure they do not
...
Restarts Shiny Server to apply configuration changes and checks that the
...
Configures Fail2ban to monitor login attempts for both RStudio Server
and Shiny Server. Specifies how failed login attempts should be detected
and defines banning policies.
...
Installs UFW and configures it to allow SSH, HTTP, HTTPS, RStudio (port
8787), and Shiny (port 3838). Verifies that UFW is active.
...
Configures NGINX to act as a reverse proxy for both RStudio and Shiny
Server. Adds support for WebSockets.
...
Enables the NGINX reverse proxy configuration, tests it, and restarts
NGINX.
...
Verifies that NGINX, RStudio Server, Shiny Server, and Fail2ban are all
running.
...
Installs the `pak` package manager and several useful R packages for
data manipulation, visualization, and development.
